...
def projective_transformer_inv(input_images, f0, f1, c0, c1, depthmap, rotation, translation, wrap_mode='border', name='projective_transformer', **kwargs):
...
#            return weight_l * pix_l + weight_r * pix_r
    ...
    def _transform(input_images, depthmap, rotation, translation, f0, f1, c0, c1):
        with tf.variable_scope('transform'):
            ...
            rt_zeros = tf.zeros((_num_batch,), dtype=tf.float32, name=None)
            rt_ones  = tf.ones((_num_batch,), dtype=tf.float32, name=None)
            ...
            images = _transform_with_Rt(input_images, Rt_mat, t_mat, xyz, f0, c0, f1, c1)

            # transform to new image plan (x transform)
            t_mat = tf.stack([translation[:,0]+0.1*rt_ones,translation[:,1],translation[:,2]],axis=1)
            t_mat = tf.reshape(t_mat,(_num_batch,3,1))
            t_mat = tf.tile(t_mat, tf.stack([1,1,_width*_height]))

            images_x = _transform_with_Rt(input_images, Rt_mat, t_mat, xyz, f0, c0, f1, c1)

            # transform to new image plan (y transform)
            t_mat = tf.stack([translation[:,0],translation[:,1]+0.1*rt_ones,translation[:,2]],axis=1)
            t_mat = tf.reshape(t_mat,(_num_batch,3,1))
            t_mat = tf.tile(t_mat, tf.stack([1,1,_width*_height]))

            images_y = _transform_with_Rt(input_images, Rt_mat, t_mat, xyz, f0, c0, f1, c1)

            # transform to new image plan (z transform)
            t_mat = tf.stack([translation[:,0],translation[:,1],translation[:,2]+0.1*rt_ones],axis=1)
            t_mat = tf.reshape(t_mat,(_num_batch,3,1))
            t_mat = tf.tile(t_mat, tf.stack([1,1,_width*_height]))

            images_z = _transform_with_Rt(input_images, Rt_mat, t_mat, xyz, f0, c0, f1, c1)
            ...
            images_roll = _transform_with_Rt(input_images, Rt_mat, t_mat, xyz, f0, c0, f1, c1)
            ...
            images_pitch = _transform_with_Rt(input_images, Rt_mat, t_mat, xyz, f0, c0, f1, c1)
            ...
            images_yaw = _transform_with_Rt(input_images, Rt_mat, t_mat, xyz, f0, c0, f1, c1)

            return [images, images_x, images_y, images_z, images_roll, images_pitch, images_yaw]

    def _transform_with_Rt(imgs, R,t,pts,f0,c0,f1,c1):

        xyz_transformed = tf.add(tf.matmul(R,pts),-tf.matmul(R,t))
        ...
        transformed = _interpolate(imgs, x_t_flat, y_t_flat)

        transformed = tf.reshape(transformed, [_num_batch, _height, _width, _num_channels])
